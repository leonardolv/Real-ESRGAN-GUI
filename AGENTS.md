# Autonomous Agent Guidelines: GUI & UX Maintenance (Real-ESRGAN-GUI)
...
- For CustomTkinter: Ensure any test instantiating window widgets destroys them cleanly (`root.destroy()`) or executes through headless test mocks.
- Always mock dialog functions (such as `filedialog.askopenfilename`, `messagebox.showinfo`).
